...
During a mutation run that updates at least one repository consumer, also converge every matching skill that already exists in a writable user-level skill root exposed by the active host or its configured skill catalog. Resolve real paths and ownership before classifying a target. Never infer user scope by broadly crawling a home directory, and exclude project, system, plugin, provider-managed, generated, and read-only installations.
...
11. Immediately before committing and pushing, rebuild the complete skill and external-registry applicability record against each freshly fetched remote default branch. After at least one repository update is pushed, converge the frozen matching user-level installation rows from the finalized canonical artifacts. Require the same complete semantic postconditions on every current remote head and `local-verified` installation. Delegate mutation, snapshots, drift reconciliation, conflict repair, mergeability, independent remote and local auditing, and nonterminal-row enforcement to `coordinate-repositories` rather than restating or replacing those mechanics here.
