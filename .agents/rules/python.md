...
- When a `TypedDict` types a module-level constant or other shared blob, build the value by **calling** the TypedDict constructor with keyword arguments when the syntax is supported across the package's declared Python versions. Otherwise, use an annotated dict literal so Python 3.11 compatibility is preserved.
