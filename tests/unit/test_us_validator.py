...
class TestUSTaxValidatorValidate:
    ...
    def test_returns_resolution_for_valid_ssn(self, us_validator: USTaxValidator) -> None:
        """Test that a valid SSN returns a summary with resolved details."""

        result = us_validator.validate("001-01-0001", TaxIdentifierType.SSN)
    ...
    def test_rejects_unsupported_types(
        ...
        with pytest.raises(UnsupportedTaxIdTypeError):
            us_validator.validate("123456789", tax_id_type)


class TestUSTaxValidatorResolveSsn:
    """Tests for the SSN resolution helper."""

    def test_resolves_known_ssn(self, us_validator: USTaxValidator) -> None:
        """Test that a known SSN resolves to issuing details."""

        validation = us_validator.resolve_ssn("001-01-0001")
